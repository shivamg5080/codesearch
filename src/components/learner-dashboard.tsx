...
const REVIEW_CHIP: Record<string, string> = {
  due: "bg-(--red-bg) text-(--red)",
  soon: "bg-(--amber-bg) text-(--amber)",
  ontrack: "bg-(--green-bg) text-(--green)",
};
...
export function LearnerDashboard({
  ...
  return (
    <div className="flex min-h-screen bg-(--bg) text-(--text)">
      {/* Sidebar */}
      <aside className="hidden w-[230px] shrink-0 flex-col border-r border-(--border) bg-(--surface) lg:flex">
        <div className="flex items-center gap-2.5 px-5 py-5">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-(--green) text-white">
            ⚡
          </div>
          ...
          {NAV.map((n) => (
            <Link
              ...
              className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition ${
                n.active
                  ? "bg-(--green-bg) font-medium text-(--green)"
                  : "text-(--body-2) hover:bg-(--hover)"
              }`}
            >
              <span className="text-(--dim)">{n.icon}</span>
              {n.label}
            </Link>
          ))}
        </nav>
        <div className="mt-auto border-t border-(--border) px-4 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-(--inset) text-xs font-semibold text-(--body-2)">
              {initials}
            </div>
            <div className="leading-tight">
              <div className="text-sm font-medium">{user.name}</div>
              <div className="text-xs text-(--dim)">Learner</div>
            </div>
            <span className="ml-auto">
              <ThemeToggle />
            </span>
          </div>
          <div className="mt-3 flex items-center gap-1.5 rounded-lg bg-(--amber-bg) px-3 py-1.5 text-xs font-medium text-(--amber)">
      ...
      <main className="min-w-0 flex-1 px-6 py-8 lg:px-10">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-(--muted)">Practice &amp; tutor analytics</p>
          </div>
          <div className="flex items-center gap-3">
            <Dropdown
          ...
          {data.kpis.map((k) => (
            ...
            >
              ...
              {k.label === "Due for review" && parseInt(k.value, 10) > 0 && (
                <Link
                  href="/reviews"
                  className="mt-1 inline-block text-xs font-semibold text-(--green) hover:text-(--green)"
                >
                  Review now →
                </Link>
              )}
        ...
        {/* Difficulty mix */}
        {data.mix.length > 0 && (
          <div className="mt-5 flex flex-wrap items-center gap-5 rounded-2xl border border-(--border) bg-(--surface) px-5 py-4">
            <span className="font-mono text-[11px] uppercase tracking-wider text-(--dim)">
              Difficulty mix
            </span>
            <div className="flex h-2.5 min-w-[200px] flex-1 overflow-hidden rounded-full bg-(--inset)">
              {data.mix.map((m) => (
                <div
                  key={m.label}
            ...
            <div className="flex items-center gap-4 text-sm">
              {data.mix.map((m) => (
                <span key={m.label} className="flex items-center gap-1.5 text-(--muted)">
                  ...
                  {m.label} <span className="font-semibold text-(--body-2)">{m.pct}%</span>
                </span>
              ))}
            </div>
          </div>
            ...
            <div className="grid gap-2 sm:grid-cols-3">
              {data.practiceNext.problems.map((p) => (
                <Link
                  key={p.id}
                  href={`/problems/${p.id}`}
                  className="group rounded-xl border border-(--border) px-4 py-3 transition hover:border-(--green-brd) hover:bg-(--green-bg)"
                >
                  <div className="truncate text-sm font-semibold group-hover:text-(--green)">
                    {p.title}
                  </div>
        ...
        {data.topics.length > 0 ? (
          <div className="rounded-2xl border border-(--border) bg-(--surface)">
            <div className="grid grid-cols-[1.6fr_0.5fr_1.2fr_1fr_0.6fr_0.7fr] gap-4 border-b border-(--border) px-5 py-3 font-mono text-[11px] uppercase tracking-wider text-(--dim)">
            ...
            {data.topics.map((t) => (
              <button
                key={t.tag}
                onClick={() => setOpen(t)}
                className="grid w-full grid-cols-[1.6fr_0.5fr_1.2fr_1fr_0.6fr_0.7fr] items-center gap-4 border-b border-(--border-soft) px-5 py-4 text-left transition last:border-0 hover:bg-(--hover)"
              >
                ...
                <span className="font-mono text-sm text-(--muted)">L{t.avgHint}</span>
          ...
          <div className="rounded-2xl border border-(--border) bg-(--surface) p-10 text-center text-(--dim)">
...
function Dropdown({
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setShow((s) => !s)}
        className="flex items-center gap-2 rounded-lg border border-(--border) bg-(--surface) px-3.5 py-2 text-sm text-(--body-2) transition hover:border-(--border-hover)"
      >
        {label}
        <span className="text-(--dim)">▾</span>
      </button>
      {show && (
        <div className="absolute right-0 z-20 mt-1.5 w-44 overflow-hidden rounded-lg border border-(--border) bg-(--surface) py-1 shadow-lg">
          {options.map((o) => (
            <button
              key={o.key}
              onClick={() => {
              ...
              className={`flex w-full items-center justify-between px-3.5 py-2 text-left text-sm transition hover:bg-(--hover) ${
                o.key === current ? "font-medium text-(--green)" : "text-(--body-2)"
              }`}
            >
              {o.label}
              {o.key === current && <span className="text-(--green)">✓</span>}
            </button>
          ))}
        </div>
      )}
...
function Drilldown({ topic, onClose }: { topic: TopicRow; onClose: () => void }) {
  ...
  const RECENT_PILL: Record<string, string> = {
    solved: "bg-(--green-bg) text-(--green)",
    attempting: "bg-(--amber-bg) text-(--amber)",
    "review-due": "bg-(--red-bg) text-(--red)",
  };
  ...
  return (
    <>
      ...
      />
      <aside className="fixed right-0 top-0 z-40 flex h-full w-full max-w-[640px] flex-col overflow-y-auto bg-(--surface) shadow-2xl animate-[slideIn_.2s_ease] sm:w-[42%]">
        <div className="flex items-start justify-between gap-4 px-6 py-5">
          <div>
            ...
            <p className="mt-0.5 font-mono text-xs text-(--dim)">{topic.slug}</p>
          </div>
          <button
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-(--border) text-(--muted) hover:bg-(--hover)"
          >
            ✕
          </button>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3 px-6">
          {PANEL_STAT.map((s) => (
            <div key={s.key} className="rounded-xl border border-(--border) px-3 py-2.5">
        ...
        <div className="grid grid-cols-2 gap-4 px-6 pt-5">
          <div className="rounded-xl border border-(--border) p-4">
            ...
            <div className="font-mono text-xs text-(--dim)">solves / day · last 12d</div>
            <div className="mt-3">
          ...
          <div className="rounded-xl border border-(--border) p-4">
            ...
            <div className="font-mono text-xs text-(--dim)">requests / day · last 12d</div>
            <div className="mt-3">
        ...
        {topic.attention.length > 0 && (
          <div className="px-6 pt-6">
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="font-mono text-xs text-(--red)">{topic.attention.length} flagged</span>
            </div>
            <div className="space-y-2">
              {topic.attention.map((a, i) => (
                <div key={i} className="rounded-xl bg-(--red-bg) px-4 py-3">
                  ...
                  <div className="ml-3.5 font-mono text-xs text-(--red)">{a.detail}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              {topic.recent.map((r, i) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="truncate font-mono text-xs text-(--dim)">{r.meta}</div>
                  </div>
                  <div className="shrink-0 text-right">
                    ...
                    <div className="mt-1 font-mono text-xs text-(--dim)">{r.when}</div>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-sm text-(--dim)">No recent activity in this topic.</p>
          )}
        </div>
      </aside>
    </>
